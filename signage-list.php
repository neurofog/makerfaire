 ...
 */
function get_schedule_list( $location ) {
    $output = '';
    ...
    while ( $query->have_posts() ) : $query->the_post();
        ...
        $output .= '<tr>';
        $output .= '<td width="250" style="max-width:250px; padding:15px 0;" valign="top">';
        $output .= '<h2 style="font-size:1.3em; color:#333; margin-top:3px;">' . esc_html( $start ) . ' &mdash; ' . esc_html( $stop ) . '</h2>';
        $output .= '</td>';
        $output .= '<td>';
        if ( ! empty( $json->presenter_name ) ) {
            ...
            $output .= '<h1 style="margin:8px 0 0; color:#444;">' . substr($names_output, 2) . '</h1>';
        }
        $output .= '<h2 style="margin-top:0;">' . get_the_title( $sched_post->ID ) . '</h2>';
    ...
    $output .= '<h1 style="margin-top:100px;">Sunday</h1>';
...
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
    ...
    <body>
        <h1 style="font-size:3em; margin:31px 0 0; max-width:75%;"><?php echo $location; ?></h1>
        ...
        <p></p>
        <p></p>
        <?php echo get_schedule_list( $location ); ?>
    </body>
</html>
