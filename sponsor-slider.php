
<div id="sponsor-area">
<h3 class="sponsor">
	Maker Faire Sponsors
</h3>
<div id="sponsorCarousel" style="height:240px;margin-top:10px;margin-bottom:10px;" class="carousel slide sponsorCarousel">
	<div class="carousel-inner" style="background-color:#fff;">
		<div class="item active">
			<div class="table-cell">
				<a href="http://www.radioshack.com/home/index.jsp"><img style="margin-left:10px;width:280px !important;" src="http://makezineblog.files.wordpress.com/2012/08/radioshack.gif?w=280" alt="RadioShack" valign="center" title="RadioShack" /></a> 
			</div>
		</div>
		<div class="item">
			<div class="table-cell">
				<a href="http://www.cognizant.com/"><img style="margin-left:10px;width:280px !important;" src="http://makezineblog.files.wordpress.com/2012/08/cognizant_logo.jpg?w=280" alt="Cognizant" valign="center" title="Cognizant" /></a> 
			</div>
		</div>
		<div class="item">
			<div class="table-cell">
				<a href="http://www.atmel.com/"><img style="margin-left:10px;width:280px !important;" src="http://makezineblog.files.wordpress.com/2012/07/atmellogo_2012_r.jpg?w=280" alt="Atmel" valign="center" title="Atmel" /></a> 
			</div>
		</div>
		<div class="item">
			<div class="table-cell">
				<a href="http://www.disneyabctv.com/web/index.aspx"><img style="margin-left:10px;width:280px !important;" src="http://makezineblog.files.wordpress.com/2012/09/disneyabc_logo_stacked.jpg?w=280" alt="Disney ABC Television" valign="center" title="Disney ABC Television" /></a> 
			</div>
</div>
		<div class="item">
<div class="table-cell">
				<a href="http://creation.redbullusa.com/"><img style="margin-left:10px;width:280px !important;" src="http://makezineblog.files.wordpress.com/2012/08/red-bull-creation-_black.jpg?w=280" alt="Red Bull Creation Challenge" valign="center" title="Red Bull Creation Challenge" /></a> 
			</div>
</div>
<div class="item">
<div class="table-cell">
				<a href="http://www.timewarnercable.com/"><img style="margin-left:10px;width:280px !important;" src="http://makezineblog.files.wordpress.com/2012/09/timewarnercable1.jpg?w=280" alt="Time Warner Cable" valign="center" title="Time Warner Cable" /></a> 
			</div>
		</div>
		<div class="item">
			<div class="table-cell">
				<a href="http://www.asus.com/"><img style="margin-left:10px;width:280px !important;" src="http://makezineblog.files.wordpress.com/2012/08/asus_logo.jpg?w=280" alt="ASUS" valign="center" title="ASUS" /></a> 
			</div>
		</div>
		<div class="item">
			<div class="table-cell">
				<a href="http://usa.autodesk.com"><img style="margin-left:10px;width:280px !important;" src="http://cdn.makezine.com/make/makerfaire/bayarea/2012/hardware/autodesk.jpg?w=280" alt="Autodesk" valign="center" title="Autodesk" /></a> 
			</div>
		</div>
		<div class="item">
			<div class="table-cell">
				<a href="http://www.crayola.com/"><img style="margin-left:10px;width:280px !important;" src="http://makezineblog.files.wordpress.com/2012/08/crayola_logo_rgb.png?w=280" alt="Crayola" valign="center" title="Crayola" /></a> 
			</div>
		</div>
		<div class="item">
			<div class="table-cell">
				<a href="http://www.deltafaucet.com/index.html"><img style="margin-left:10px;width:280px !important;" src="http://makezineblog.files.wordpress.com/2012/04/deltalogo_4c_black_tag-copy.png?w=280" alt="Delta Faucet" valign="center" title="Delta Faucet" /></a> 
			</div>
		</div>
		<div class="item">
			<div class="table-cell">
				<a href="http://www.epiloglaser.com/"><img style="margin-left:10px;width:280px !important;" src="http://makezineblog.files.wordpress.com/2012/09/epilog.jpg?w=280" alt="Epilog Laser" valign="center" title="Epilog Laser" /></a> 
			</div>
		</div>
<div class="item">
			<div class="table-cell">
				<a href="http://www.ge.com/"><img style="margin-left:10px;width:280px !important;" src="http://makezineblog.files.wordpress.com/2012/10/ge-logo.jpg" alt="General Electric" valign="center" title="General Electric" /></a> 
			</div>
</div>
	<div class="item">
			<div class="table-cell">
				<a href="http://www.gm.com/"><img style="margin-left:10px;width:280px !important;" src="http://makezineblog.files.wordpress.com/2012/09/general-motors-company-copy.jpg?w=280" alt="General Motors" valign="center" title="General Motors" /></a> 
			</div>
</div>
<div class="item">
			<div class="table-cell">
				<a href="https://plus.google.com/"><img style="margin-left:10px;width:280px !important;" src="http://makezineblog.files.wordpress.com/2012/09/google-plus-logo-05.jpeg?w=280" alt="Google Plus" valign="center" title="Google Plus" /></a> 
			</div>
</div>
		<div class="item">
			<div class="table-cell">
				<a href="facebook.com/schickxtreme3"><img style="margin-left:10px;width:280px !important;" src="http://makezineblog.files.wordpress.com/2012/08/schick_x3_eco_m-logo-hr.jpg?w=280" alt="Schick Xtreme 3" valign="center" title="Schick Xtreme 3" /></a> 
			</div>
		</div>
		<div class="item">
			<div class="table-cell">
				<a href="http://www.shopbottools.com/"><img style="margin-left:10px;width:280px !important;" src="http://makezineblog.files.wordpress.com/2012/03/shopbot-logo-blue-w-reg-cmyk-copy-copy.png?w=280" alt="ShopBot Tools" valign="center" valign="center" title="ShopBot Tools" /></a> 
			</div>
		</div>
		<div class="item">
			<div class="table-cell">
				<a href="http://www.sketchup.com/"><img style="margin-left:10px;width:280px !important;" src="http://makezineblog.files.wordpress.com/2012/09/sketchup_logo_hires.jpeg?w=280" alt="Sketch Up" valign="center" title="Sketch Up" /></a> 
			</div>
	</div>
<div class="item">
			<div class="table-cell">
				<a href=""><img style="margin-left:10px;width:280px !important;" src="http://makezineblog.files.wordpress.com/2012/10/makerweekribbon1.jpeg?w=280" alt="" valign="center" title="Maker Week" /></a> 
			</div>
		</div>
	</div>
	<!-- END sponsor-area -->
</div>